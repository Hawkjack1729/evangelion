...
def detect_Qr_details(image):
    ...
    for qr in pyzbar.decode(image):
        ...
        x = int((x1 + x2)/2)
        y = int((y1+y2)/2)
        center_coordinate.append(x)
        center_coordinate.append(y)
        data = qr.data.decode('utf-8')
        details = {data: center_coordinate}
        Qr_codes_details.update(details)
    ##################################################
...
def detect_ArUco_details(image):
    ...
    corners, ids, _ = aruco.detectMarkers(gray_img, aruco_dict, parameters=parameter)
    points = np.array(corners, np.int32)
    points_list = points.tolist()
    for i in points_list:
        c = []
        d = []
        x1, x2, x3, x4 = i[0][0][0], i[0][1][0], i[0][2][0], i[0][3][0]
        y1, y2, y3, y4 = i[0][0][1], i[0][1][1], i[0][2][1], i[0][3][1]
        p1, p2, p3, p4 = [x1, y1], [x2, y2], [x3, y3], [x4, y4]
        x_cen1 = (x1 + x3)/2
        x_cen2 = (x2 + x4)/2
        y_cen1 = (y1+y3)/2
        y_cen2 = (y2+y4)/2
        x = int((x_cen1+x_cen2)/2)
        y = int((y_cen1+y_cen2)/2)
        d.append(x)
        d.append(y)
        c.append(d)
        pi = math.pi
        angle = (math.atan((y2-y1)/(x2-x1)))*180/pi
        if x1 == min(x1, x2, x3, x4):
            c.append(int(-angle))
        elif x1 == max(x1, x2, x3, x4):
            c.append(int(-angle-180))
        elif y1 == min(y1, y2, y3, y4):
            c.append(int(-angle))
        elif y1 == max(y1, y2, y3, y4):
            c.append(int(-angle+180))
        id = int(ids[points_list.index(i)])
        details = {id: c}
        corners_details = {id: [p1, p2, p3, p4]}
        ArUco_details_dict.update(details)
        ArUco_corners.update(corners_details)
    ##################################################
